<template>
  <div class="results-panel">
    ...
    <span v-if="results[0]" class="reward">
      ...
      <br>
      <span v-tooltip="convertWei(results[4])+' USDT'">{{"and "+formattedDkc}}</span>
        <Hint text="USDT earned is based on gas costs of the network plus a factor of your power" />
    </span>
  </div>
</template>
...
export default {
  ...
  computed: {
    formattedDkc() {
      const dkcBalance = fromWeiEther(this.results[4]);
      return `${toBN(dkcBalance).toFixed(6)} USDT`;
    }
  },

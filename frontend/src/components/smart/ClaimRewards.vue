<template>
  <div class="body main-font">
    <b-navbar-nav>
      ...
      <b-nav-item-dropdown right>
        ...
        <b-dropdown-item
          ...
          @click="onClaimTokens()" class="gtag-link-others" tagname="claim_dkc">
            DKC
            <div class="pl-3">{{ formattedDkcReward }}</div>
            <div class="pl-3">
    ...
    <b-modal class="centered-modal" ref="need-gas-modal" title="Need Withdraw?"
      @ok="claimDkc(ClaimStage.Stake)" ok-title="Next" @cancel="$router.push({ name: 'portal' })" cancel-title="Go to WAX Portal" >
    ...
    <b-modal class="centered-modal" ref="stake-suggestion-modal" title="Stake Dkc"
      @ok="$router.push({ name: 'select-stake-type' })" ok-only ok-title="Go to Stake" >
        You can avoid paying the 15% tax by staking unclaimed dkc rewards for 7 days. If you stake your DKC now, we'll give you a
        50% bonus in-game only DKC that you can use right away!
      <a href="#" @click="claimDkc(ClaimStage.Claim)"> <br>No thanks, I'd rather {{ (this.rewardsClaimTaxAsFactorBN > 0)?"pay " +
        this.formattedTaxAmount + " in taxes and " : ""  }}forfeit my bonus </a>
    </b-modal>
    <b-modal class="centered-modal" ref="claim-confirmation-modal" title="Claim Dkc" ok-title="I am sure"
      @ok="onClaimTokens()"> You are about to {{ (this.rewardsClaimTaxAsFactorBN > 0)?"pay " + formattedRewardsClaimTax +
      " tax for early withdrawal, costing you " + this.formattedTaxAmount + " DKC. You will also " : "" }}
      miss out on {{formattedBonusLost}} bonus DKC. Are you sure
...
export default Vue.extend({
  ...
  computed: {
    ...(mapState(['dkcRewards', 'xpRewards', 'ownedCharacterIds', 'directStakeBonusPercent']) as Accessors<StoreMappedState>),
    ...(mapGetters([
      'ownCharacters', 'maxRewardsClaimTaxAsFactorBN', 'rewardsClaimTaxAsFactorBN'
    ]) as Accessors<StoreMappedGetters>),

    formattedDkcReward(): string {
      const dkcRewards = fromWeiEther(this.dkcRewards);
      return `${toBN(dkcRewards).toFixed(4)}`;
    },
    ...
    formattedTaxAmount(): string {
      const dkcRewards = fromWeiEther(toBN(parseFloat(this.dkcRewards)* parseFloat(String(this.rewardsClaimTaxAsFactorBN))).toString());
      return `${toBN(dkcRewards).toFixed(4)}`;
    },

    formattedBonusLost(): string {
      const dkcLost = fromWeiEther(toBN(parseFloat(this.dkcRewards)*this.directStakeBonusPercent/100).toString());
      return `${toBN(dkcLost).toFixed(4)}`;
    },
  ...
  methods: {
    ...(mapActions(['addMoreDkc', 'claimTokenRewards', 'claimXpRewards']) as StoreMappedActions),
    ...
    async claimDkc(stage: ClaimStage) {

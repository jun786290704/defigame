      ...
      >
        <div class="reward-claim-inner-wrapper">
          ...
          <p class="center-text selectable">
            {{ currentRewardEarned.toFixed(18) }} DKC
          </p>
      ...
      <div class="stakePage medium-dark-bg">
        ...
        >
          ...
          <span class="gold-text" v-else-if="canStakeUnclaimedRewards">
            Stake all of unclaimed rewards ({{ formattedDkcRewards }} DKC)
          </span>
          <span class="gold-text" v-else>
...
export default {
  ...
  computed: {
    ...mapState(['defaultAccount', 'staking', 'dkcRewards']),
    ...
    stakingTokenName() {
      return this.stakeType === 'dkc' || this.stakeType === 'dkc2' ? 'DKC' : 'DKC-WBNB';
    },
    ...
    canStakeUnclaimedRewards() {
      return !this.loading && toBN(this.dkcRewards).gt(0);
      // return true;
    },

    formattedDkcRewards() {
      const b = toBN(this.dkcRewards);

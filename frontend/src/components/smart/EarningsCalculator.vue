              ...
              <div class="calculator-earnings">
                <div class="coin-price-inputs">
                  ...
                  <div class="prices-div">
                    ...
                    <div class="token-price-div">
                     DKC:  <b-form-label class="price-input" type="number" v-model="dkcPrice" /> <span class="text-white"> ${{dkcPrice }}</span>
                    </div>
                  </div>
                </div>
                <div class="results">
                ...
                <div class="button-div">
                  ...
                  <b-icon-question-circle class="centered-icon" scale="1.5"
                    v-tooltip.bottom="`Earnings on victory: ${this.stringFormattedDkc(this.fightGasOffset)} gas offset +
                    ${this.stringFormattedDkc(this.fightBaseline)} per square root of power/1000`"/>
                </div>
              </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async fetchPrices() {
      ...
      this.dkcPrice = data?.cryptoblades.usd;
    },
    ...
    calculateEarnings() {
      ...
      const averageDailyReward = this.getAverageRewardForPower(totalPower) *7.2 +
        this.formattedDkc(this.fightGasOffset) * fights;
      const averageFightProfit = averageDailyReward * this.dkcPrice / 7.2;
      for(let i = 1; i < 8; i++) {
    ...
    getAverageRewardForPower(power: number): number {
      return (this.formattedDkc(this.fightBaseline) * Math.sqrt(power / 1000));
    },
    ...
    getAverageRewardAtLevel(level: number): number {
      return this.formattedDkc(this.fightGasOffset) + (this.formattedDkc(this.fightBaseline) * (Math.sqrt(CharacterPower(level - 1)/1000)));
    },
    ...
    formattedDkc(dkc: number): number {
      const dkcBalance = fromWeiEther(dkc.toString());
      return toBN(dkcBalance).toNumber();
    },

    stringFormattedDkc(dkc: number): string {
      const dkcBalance = fromWeiEther(dkc.toString());
      return toBN(dkcBalance).toFixed(6);
    },

<template>
  <div class="d-flex">
    <div class="sidebar">
      <div class="content body main-font">
        <div class="blank-slate" v-if="ownWeapons.length === 0">
          ...
          <big-button
            class="button"
            :mainText="`Forge sword for ${forgeCost} DKC`"
            @click="onForgeWeapon"
          />
        </div>
              ...
              <div class="d-flex justify-content-flex-end ml-auto">
                ...
                <b-button
                  ...
                  <span v-if="!disableForge" class="gtag-link-others" tagname="forge_weapon">
                    Forge x1 ({{ forgeCost }} DKC) <i class="fas fa-plus"></i>
                  </span>
                </b-button>
                ...
                <b-button
                  ...
                  <span v-if="!disableForge" class="gtag-link-others" tagname="forge_weapon">
                    x10 ({{ forgeCost*10 }} DKC) <i class="fas fa-plus"></i>
                  </span>
                </b-button>
                <b-icon-question-circle
                class="centered-icon ml-3"
                scale="1.5"
                v-on:click="onShowForgeDetails"
                v-tooltip.bottom="'Click for forge percentages'"
                />

                <b-modal hide-footer ref="forge-details-modal" title="Forge Percentages">
                  <div>
                    5+ star @ 1% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.01)).toFixed(2)}} DKC.
                  </div>
                  <div>
                    4+ star @ 6% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.06)).toFixed(2)}} DKC.
                  </div>
                  <div>
                    3+ star @ 21% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.21)).toFixed(2)}} DKC.
                  </div>
                  <div>
                    2+ star @ 56% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.56)).toFixed(2)}} DKC.
                  </div>
                  <div>
                    1+ star @ 100% chance.
                  </div>
          ...
          <div class="col-md-12" v-if="showReforge">
            <b-modal class="centered-modal text-center" ref="reforge-confirmation-modal" title="Reforge Confirmation" @ok="onReforgeWeapon">
              ...
              <div class="text-center" :hidden="burnWeaponId === null || !isWeaponReforged()">
                <b-icon icon="exclamation-circle" variant="danger" />[WARNING] This item has been previously reforged
                 and only half of each bonus will carry over!
              </div>
              <div class="row">
                <div class="headings">
...
export default {
  ...
  async created() {
    ...
    const dkcForgeCost = await this.contracts.CryptoBlades.methods.usdToDkc(forgeCost).call();
    this.forgeCost = BN(dkcForgeCost).div(BN(10).pow(18)).toFixed(4);

    const reforgeCost = await this.contracts.CryptoBlades.methods.reforgeWeaponFee().call({ from: this.defaultAccount });
    const dkcReforgeCost = await this.contracts.CryptoBlades.methods.usdToDkc(reforgeCost).call();
    this.reforgeCost = BN(dkcReforgeCost).div(BN(10).pow(18)).toFixed(4);
  },
...
.content {
  padding: 0 !important;
  transform: scale(0.5); /* 缩小内容到原来的一半大小 */
}

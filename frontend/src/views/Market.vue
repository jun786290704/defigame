                    ...
                    <template #above="{ weapon: { id } }">
                      <div class="d-flex flex-column align-items-center justify-content-center m-top-negative-5">
                        <span class="d-block text-center fix-h24" v-if="nftPricesById[id]">
                          <span v-if="convertWeiToDkc(nftPricesById[id]) !== '0'"
                          v-tooltip.top="{ content: maxPrecisionDkc(nftPricesById[id]) , trigger: (isMobile() ? 'click' : 'hover') }"
                          @mouseover="hover = !isMobile() || true"
                          @mouseleave="hover = !isMobile()"
                          >
                            <strong>Price</strong>: {{ convertWeiToDkc(nftPricesById[id]) | dynamicDecimals(2, 4) }} DKC
                          </span>
                        </span>
                        <span class="d-block text-center" v-else>Loading price...</span>
                        <b-button
                          :hidden="convertWeiToDkc(nftPricesById[id]) === '0'"
                          @click="selectedNftId = id; purchaseNft();"
                          variant="primary"
                          class="gtag-link-others">
                          {{ convertWeiToDkc(nftPricesById[id]) !== '0' ? 'Purchase' : 'Sold' }}
                        </b-button>
                      </div>
                    </template>

                    <template #sold="{ weapon: { id } }">
                      <div class="sold" v-if="nftPricesById[id] && convertWeiToDkc(nftPricesById[id]) === '0'"><span>sold</span></div>
                    </template>
                    ...
                    <template #above="{ character: { id } }">
                      <div class="token-price d-flex flex-column align-items-center justify-content-center m-top-negative-50">
                        <span class="d-block text-center fix-h24" v-if="nftPricesById[id]">
                          ...
                          @mouseleave="hover = !isMobile()"
                          >
                          {{ convertWeiToDkc(nftPricesById[id]) | dynamicDecimals(2, 4) }} DKC
                          </span>
                        </span>
                        ...
                        <b-button
                          ...
                          {{ convertWeiToDkc(nftPricesById[id]) !== '0' ? 'Purchase' : 'Sold' }} <b-icon-question-circle v-if="!canPurchase"
                ...
                <div class="row button-row">
                  ...
                  <div class="col-4 col-md-3 col-lg-2 mb-2">
                    ...
                    <b-modal class="centered-modal" ref="listing-setup-modal"
                      ...
                      <b-form-input type="number" :max="10000"
                        class="modal-input" v-model="listingSellPrice" placeholder="Sell Price (DKC)" />

                      <span v-if="listingSellPrice">Do you want to sell your {{activeType}} for {{Math.min(+listingSellPrice, 10000)}} DKC?<br>
                      <i>The buyer will pay an extra {{activeListingMarketTax()}}% market fee for a total of
                      {{calculatedBuyerCost(Math.min(+listingSellPrice, 10000))}} DKC</i></span>
                    </b-modal>
                  </div>
                  ...
                  <div class="col-4 col-md-3 col-lg-2">
                    ...
                    <b-modal class="centered-modal " ref="characters-sold-modal">
                        ...
                        <div v-if="historyCounter > 0">
                          <b-table
                           class="transaction-history-text" :items="characterTransactionHistoryData" :fields="characterTransactionHistoryHeader">
                          </b-table>
                        </div>
                        <div v-if="historyCounter === 0">
                          <p>It's seems like there's nothing here.</p>
  ...
  methods: {
    ...
    async addListingForNft() {
      ...
      const results = await this.addMarketListing({
        ...
        price: this.convertDkcToWei(val.toString()),
      });

      this.selectedNftId = null;
      this.waitingMarketOutcome = false;
      this.marketOutcome = 'Successfully listed '
        +this.activeType+' '+results.nftID+' for '+this.convertWeiToDkc(results.price)+' DKC';
    },
    ...
    async updateNftListingPrice() {
      ...
      const results = await this.changeMarketListingPrice({
        nftContractAddr: this.contractAddress,
        tokenId: this.selectedNftId,
        newPrice: this.convertDkcToWei(val.toString())
      });

      this.selectedNftId = null;
      this.waitingMarketOutcome = false;
      this.marketOutcome = 'Successfully changed price for '
        +this.activeType+' '+results.nftID+' to '+this.convertWeiToDkc(results.newPrice)+' DKC';
    },
    ...
    async purchaseNft() {
      ...
      const dkcChainPrice = this.convertStringToDecimal(this.convertWeiToDkc(price), 2);
      const dkcUIPrice = this.convertStringToDecimal(this.convertWeiToDkc(this.nftPricesById[this.selectedNftId]), 2);

      if(dkcChainPrice !== dkcUIPrice) {
    ...
    convertWeiToDkc(wei: string) {
      return fromWeiEther(wei);
    },
    convertDkcToWei(dkc: string) {
      return Web3.utils.toWei(dkc);
    },
    ...
    maxPrecisionDkc(listedPrice: string): string {
      return this.convertStringToDecimal(this.convertWeiToDkc(listedPrice), 8);
    }
  },

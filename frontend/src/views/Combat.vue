...
export default {
  ...
  methods: {
    ...mapActions(['fetchTargets', 'doEncounter', 'fetchFightRewardDkc', 'fetchFightRewardXp', 'getXPRewardsIfWin']),
    ...
    async onClickEncounter(targetToFight) {
      ...
      try {
        ...
        await this.fetchFightRewardDkc();
    ...
    formattedDkc(dkc) {
      const dkcBalance = fromWeiEther(dkc, 'ether');
      return `${toBN(dkcBalance).toFixed(6)} DKC`;
    },
...
.enemy-character {
  ...
  align-items: center;
  margin-bottom: 5px;
  box-shadow: 0px 6px 8px rgba((0, 0, 0, 0.705), 0px 12px 7px rgba(0, 0, 0, 0.5), 0px 9px 12px rgba(233, 14, 14, 0.1)0.1);
}
/* 战斗选择的框框大小 */
.encounter img {
  width: 110px;

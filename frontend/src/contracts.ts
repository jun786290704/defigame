...
import { networks as dkcStakingRewardsNetworks } from '../../build/contracts/DkcStakingRewardsUpgradeable.json';
...
import { networks as dkcTokenNetworks } from '../../build/contracts/DkcToken.json';
...
const stakingContractAddressesFromBuild: Partial<Record<StakeType, Partial<StakingContractEntry>>> = {
  dkc: {
    stakingRewardsAddress: (dkcStakingRewardsNetworks as Networks)[networkId]?.address,
    stakingTokenAddress: (dkcTokenNetworks as Networks)[networkId]?.address
  },
  lp: {
...
async function setUpStakingContracts(web3: Web3) {
  ...
  const dkcTokenAddress = process.env.VUE_APP_DKC_TOKEN_CONTRACT_ADDRESS || (dkcTokenNetworks as Networks)[networkId]!.address;
  const DkcToken = new web3.eth.Contract(erc20Abi as Abi, dkcTokenAddress);

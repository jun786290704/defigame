...
import { approveFee, getFeeInDkcFromUsd } from './contract-call-utils';
...
interface StakingContracts {
  ...
  RewardToken: Contracts['DkcToken'],
}
...
export function createStore(web3: Web3) {
  return new Vuex.Store<IState>({
    state: {
      ...
      staking: {
        dkc: { ...defaultStakeState },
        dkc2: { ...defaultStakeState },
        lp: { ...defaultStakeState },
        lp2: { ...defaultStakeState }
      },
      stakeOverviews: {
        dkc: { ...defaultStakeOverviewState },
        dkc2: { ...defaultStakeOverviewState },
    ...
    mutations: {
      ...
      updateDkcBalance(state: IState, { dkcBalance }) {
        state.dkcBalance = dkcBalance;
      },

      updateDkcRewards(state: IState, { dkcRewards }: { dkcRewards: string }) {
        state.dkcRewards = dkcRewards;
      },
    ...
    actions: {
            ...
            state.contracts().Characters!.events.NewCharacter(
              ...
              async (err: Error, data: any) => {
                ...
                await Promise.all([
                  ...
                  dispatch('fetchDkcBalance'),
                  dispatch('fetchFightRewardDkc'),
          ...
          if(NFTMarket) {
            subscriptions.push(
              NFTMarket.events.PurchasedListing({ filter: { seller: state.defaultAccount } }, async (err: Error, data: any) => {
                ...
                await dispatch('fetchDkcBalance');
              })
            );
          }
      ...
      async fetchUserDetails({ dispatch }) {
        const promises = [dispatch('fetchDkcBalance'), dispatch('fetchWaxBridgeDetails')];
      ...
      async fetchDkcBalance({ state, commit, dispatch }) {
        const { defaultAccount } = state;
        if(!defaultAccount) return;

        await Promise.all([
          (async () => {
            const dkcBalance = await state.contracts().DkcToken.methods
              .balanceOf(defaultAccount)
              .call(defaultCallOptions(state));

            if(state.dkcBalance !== dkcBalance) {
              commit('updateDkcBalance', { dkcBalance });
            }
          })(),
          dispatch('fetchInGameOnlyFunds')
        ]);
      ...
      async addMoreDkc({ state, dispatch }, dkcToAdd: string) {
        if(featureFlagStakeOnly) return;

        await state.contracts().CryptoBlades!.methods.recoverDkc(dkcToAdd).send({
      ...
      async mintWeaponN({ state, dispatch }, {num}) {
        const { CryptoBlades, DkcToken, Weapons } = state.contracts();
        if(!CryptoBlades || !DkcToken || !Weapons || !state.defaultAccount) return;
      ...
      async fetchStakeOverviewDataPartial({ state, commit }, { stakeType }: { stakeType: StakeType }) {
        ...
        const stakeDkcOverviewData: IStakeOverviewState = {
          rewardRate,
          rewardsDuration: parseInt(rewardsDuration, 10),
          totalSupply,
          minimumStakeTime: parseInt(minimumStakeTime, 10),
        };
        commit('updateStakeOverviewDataPartial', { stakeType, ...stakeDkcOverviewData });
      },
      ...
      async fetchMarketNftPrice({ state }, { nftContractAddr, tokenId }) {
        ...
        // returns the listing's price in dkc wei
        return await NFTMarket.methods
          .getFinalPrice(
            nftContractAddr,
            tokenId
      ...
      async purchaseMarketListing({ state, dispatch }, { nftContractAddr, tokenId, maxPrice }: { nftContractAddr: string, tokenId: string, maxPrice: string }) {
        const { DkcToken, NFTMarket, Weapons, Characters } = state.contracts();
        if(!NFTMarket || !Weapons || !Characters) return;

        await DkcToken.methods
      ...
      async fetchFightGasOffset({ state, commit }) {
        ...
        const fightGasOffset = await getFeeInDkcFromUsd(
          CryptoBlades,
          defaultCallOptions(state),
      ...
      async fetchFightBaseline({ state, commit }) {
        ...
        const fightBaseline = await getFeeInDkcFromUsd(
          CryptoBlades,
          defaultCallOptions(state),
      ...
      async fetchFightRewardDkc({ state, commit, dispatch }) {
        const { CryptoBlades } = state.contracts();
        if(!CryptoBlades) return;

        const [dkcRewards] = await Promise.all([
          (async () => {
            const dkcRewards = await CryptoBlades.methods
              .getTokenRewards()
              .call(defaultCallOptions(state));

            commit('updateDkcRewards', { dkcRewards });

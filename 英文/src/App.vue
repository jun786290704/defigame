<template>
  <div class="app">
    ...
    <div
      ...
      v-if="!hideWalletWarning && !showMetamaskWarning && (errorMessage || (ownCharacters.length === 0 && dkcBalance === '0' && !hasStakedBalance))"
    >
      <div class="starter-panel">
        <img class="mini-icon-starter" src="./assets/placeholder/sword-placeholder-6.png" alt="" srcset="" />
        <span class="starter-panel-heading">{{ errorMessage || '龙的游戏' }}</span>
        <img class="mini-icon-starter" src="./assets/placeholder/sword-placeholder-6.png" alt="" srcset="" />
        <div>
          <big-button class="button mm-button" :mainText="`配置网络`" @click="configureMetaMask" />
          <big-button v-bind:class="[isConnecting ? 'disabled' : '']" class="button mm-button" :mainText="`连接钱包`" @click="connectMetamask" />
        </div>
        <div class="seperator"></div>
        <div class="instructions-list">
          <p>
            在不到10分钟的时间内开始！要招募你的第一个角色，你需要{{recruitCost}}DKC和.001 BNB的汽油。您还需要.0015

            BNB做你的前几场战斗，但别担心，你会立即在DKC奖励中赚取战斗费！
          </p>
          <!-- <ul class="unstyled-list">
            <li>1. Buying BNB with fiat: <a href="https://youtu.be/6-sUDUE2RPA" target="_blank" rel="noopener noreferrer">Watch Video</a></li>
            <li>
              2. Once you have BNB, go to ApeSwap to obtain DKC tokens:<br />
              <a v-bind:href="`${getExchangeUrl}`" target="_blank">Trade DKC/BNB</a>
            </li>
            <li>
              3. Follow this tutorial to swap BNB for DKC: <a href="https://youtu.be/_zitrvJ7Hl4" target="_blank" rel="noopener noreferrer">Watch Video</a>
            </li>
            <li>
        ...
        <div class="seperator"></div>
        <small-button class="button" @click="toggleHideWalletWarning" :text="'隐藏'" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(['dkcBalance', 'defaultAccount', 'currentNetworkId', 'currentCharacterId', 'staking']),
  ...
  methods: {
    ...
    async initializeRecruitCost() {
      ...
      this.recruitCost = BN(dkcRecruitCost)
        .div(BN(10).pow(18))
        .toFixed(4);
    },
    data() {
    ...
    async showWarningDialog() {
      ...
      if (
        ...
        (this.errorMessage || this.showNetworkError || (this.ownCharacters.length === 0 && this.dkcBalance === '0' && !this.hasStakedBalance))
      ) {
...
body {
  margin: 0;
  background: linear-gradient(45deg, rgb(22, 22, 22)100%, rgba(36, 39, 32, 1) 100%);
}
...
.outline {
  color: rgb(12, 12, 12)dfd;
  text-shadow: -1px 0 #fff, 0 1px #fff, 1px 0 #fff, 0 -1px #fff;
}

.black-outline {
  text-shadow: -1px 0 #020202, 0 1px #000, 1px 0 #000, 0 -1px #000;
}

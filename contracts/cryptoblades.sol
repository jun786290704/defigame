...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    IERC20 public dkcToken;//0x6E6f68124B2E70AE1Af19616DFbb4E42BA572DE8;
    IERC20 public usdtToken;
    IPriceOracle public priceOracleDkcPerUsd;
    IRandoms public randoms;

    function initialize(IERC20 _dkcToken, IERC20 _usdtToken, Characters _characters, Weapons _weapons, IPriceOracle _priceOracleDkcPerUsd, IRandoms _randoms) public initializer {
    ...
    mapping(address => uint256) tokenRewards; // user adress : usdt wei
    ...
    event InGameOnlyFundsGiven(address indexed to, uint256 dkcAmount);

    function recoverDkc(uint256 amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");

        dkcToken.safeTransfer(msg.sender, amount);
    }

    function getDkcToSubtract(uint256 _inGameOnlyFunds, uint256 _tokenRewards, uint256 _usdtNeeded)
        public
        pure
        returns (uint256 fromInGameOnlyFunds, uint256 fromTokenRewards, uint256 fromUserWallet) {

        if(_usdtNeeded <= _inGameOnlyFunds) {
            return (_usdtNeeded, 0, 0);
        }

        _usdtNeeded -= _inGameOnlyFunds;

        if(_usdtNeeded <= _tokenRewards) {
            return (_inGameOnlyFunds, _usdtNeeded, 0);
        }

        _usdtNeeded -= _tokenRewards;

        return (_inGameOnlyFunds, _tokenRewards, _usdtNeeded);
    }

    function getUsdtNeededFromUserWallet(address playerAddress, uint256 usdtNeeded)
        public
        view
        returns (uint256 usdtNeededFromUserWallet) {

        (,, usdtNeededFromUserWallet) = getDkcToSubtract(
    ...
    function performFight(
        ...
        uint256 tokens = usdToDkc(getTokenGainForFight(targetPower, fightMultiplier));
    ...
    function mintCharacter() public onlyNonContract oncePerBlock(msg.sender) {
        ...
        uint256 dkcAmount = usdToDkc(mintCharacterFee);
        ...
        require(dkcToken.balanceOf(msg.sender) >= fromUserWallet,
            string(abi.encodePacked("Not enough DKC! Need ",RandomUtil.uint2str(fromUserWallet))));
        ...
        if(!promos.getBit(msg.sender, promos.BIT_FIRST_CHARACTER()) && characters.balanceOf(msg.sender) == 0) {
            _giveInGameOnlyFundsFromContractBalance(msg.sender, usdToDkc(promos.firstCharacterPromoInGameOnlyFundsGivenInUsd()));
        }
    ...
    } //这个函数允许合约的外部调用者（通过msg.sender表示）向指定的玩家地址（playerAddress）支付一定数量的转换后的代币（convertedAmount）。这个函数被标记为restricted，这意味着它只能在合约内部被调用，这通常是为了防止外部调用者直接触发这个函数，从而确保只有合约的特定部分（如合约的拥有者或管理员）可以执行这个操作。
    ...
    } //它允许合约的调用者（在这种情况下是合约本身）向玩家（playerAddress）支付一定数量的代币，并且这个代币金额是经过转换的（convertedAmount）。这个函数的目的是提供一个受限制的接口，确保只有合约本身可以调用这个函数来支付代币。
    ...
    function _payContractTokenOnly(address playerAddress, int128 usdAmount) internal {
        uint256 convertedAmount = usdToDkc(usdAmount);
        ...
        dkcToken.safeTransferFrom(playerAddress, address(this), fromUserWallet);
    }
    ...
    function _payContractConverted(address playerAddress, uint256 convertedAmount) internal {
        ...
        tokenRewards[playerAddress] = tokenRewards[playerAddress].sub(fromTokenRewards);
        usdtToken.safeTransferFrom(playerAddress, address(this), fromUserWallet);
    }// 它负责处理向玩家（playerAddress）支付转换后的代币金额（convertedAmount）。这个函数在内部调用，并且涉及到从游戏内资金、代币奖励和用户钱包中扣除相应的金额。
    ...
    function _payPlayerConverted(address playerAddress, uint256 convertedAmount) internal {
        usdtToken.safeTransfer(playerAddress, convertedAmount);
    }
    ...
    function giveInGameOnlyFunds(address to, uint256 usdtAmount) external restricted {
        totalInGameOnlyFunds = totalInGameOnlyFunds.add(usdtAmount);
        inGameOnlyFunds[to] = inGameOnlyFunds[to].add(usdtAmount);

        dkcToken.safeTransferFrom(msg.sender, address(this), usdtAmount);

        emit InGameOnlyFundsGiven(to, usdtAmount);
    }

    function _giveInGameOnlyFundsFromContractBalance(address to, uint256 usdtAmount) internal {
        totalInGameOnlyFunds = totalInGameOnlyFunds.add(usdtAmount);
        inGameOnlyFunds[to] = inGameOnlyFunds[to].add(usdtAmount);
    ...
    function giveInGameOnlyFundsFromContractBalance(address to, uint256 usdtAmount) external restricted {
        _giveInGameOnlyFundsFromContractBalance(to, usdtAmount);
    }
    ...
    function stakeUnclaimedRewards() external {
        ...
        usdtToken.approve(address(stakeFromGameImpl), _tokenRewards);
    ...
    function getTotalDkcOwnedBy(address wallet) public view returns (uint256) {
        return dkcToken.balanceOf(wallet);
    }
    //这个函数的设计逻辑是将玩家在游戏内的DKC资金、代币奖励和用户钱包中的DKC代币余额相加，以得到玩家总共拥有的DKC数量。
    function getTotalusdtOwnedBy(address wallet) public view returns (uint256) {
        return inGameOnlyFunds[wallet] + getTokenRewardsFor(wallet) + usdtToken.balanceOf(wallet);
    } //这个函数的设计逻辑是将玩家在游戏内的DKC资金、代币奖励和用户钱包中的DKC代币余额相加，以得到玩家总共拥有的DKC数量。这个函数对于玩家来说是有用的，因为它提供了一个快速的方式来查看他们的总资产。由于这个函数是view类型的，它不会对合约的状态产生任何影响，因此可以安全地被频繁调用。

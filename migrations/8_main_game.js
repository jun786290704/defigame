...
module.exports = async function (deployer, network) {
  let randoms, dkcToken, usdtToken;
  if (network === 'development' || network === 'development-fork') {
 
    randoms = await deployProxy(DummyRandoms, [], { deployer });
    usdtToken = await IERC20.at('0x9E82D320D9496539B6667f01a0E0d118E1dB063E');

    dkcToken = await DkcToken.deployed();
    ...
    dkcToken = await IERC20.at('0x154a9f9cbd3449ad22fdae23044319d6ef2a1fab');
  }

  assert(dkcToken != null, 'Expected dkcToken to be set to a contract');
  ...
  const game = await deployProxy(CryptoBlades, [dkcToken.address, usdtToken.address, charas.address, weps.address, priceOracle.address, randoms.address], { deployer });

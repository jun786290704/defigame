const DkcStakingRewards = artifacts.require("DkcStakingRewards");
...
module.exports = async function (deployer, network) {
  if (network === 'bscmainnet' || network === 'bscmainnet-fork') {
    ...
    await deployer.deploy(DkcStakingRewards, ownerAddress, rewardDistributorAddress, dkcTokenAddress, dkcTokenAddress, 1000000000000000, 7 * 24 * 60 * 60);

    // 0.001 minimumStakeAmount
    await deployer.deploy(LPStakingRewards, ownerAddress, rewardDistributorAddress, dkcTokenAddress, lpTokenAddress, 1000000000000000, 0);
  }
};

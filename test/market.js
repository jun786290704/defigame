...
contract('NFTMarket', accounts => {
  let market, dkc, characters;
  beforeEach(async () => {
    dkc = await DkcToken.deployed();
    characters = await Characters.deployed();

    market = await NFTMarket.new();

    await market.initialize(dkc.address, accounts[2]);
    ...
    await dkc.transferFrom(dkc.address, accounts[1], web3.utils.toWei('10', 'ether'));
  });
  ...
  it('supports buying', async () => {
    ...
    const accountDkcBalancesPre = await Promise.all(
      accounts.map(acc => dkc.balanceOf(acc).then(toBN))
    );

    await dkc.approve(market.address, web3.utils.toWei('6', 'ether'), { from: accounts[1] });
    ...
    const accountDkcBalancesPost = await Promise.all(
      accounts.map(acc => dkc.balanceOf(acc).then(toBN))
    );

    const accountDkcBalanceDiffs = accountDkcBalancesPost.map((bn, i) =>
      bn.minus(accountDkcBalancesPre[i]).div(web3.utils.unitMap['ether'])
    );

    assert.strictEqual(
      accountDkcBalanceDiffs[0].toFixed(1),
      '5.0',
      'Expected seller to recieve original price'
    );
    assert.strictEqual(
      accountDkcBalanceDiffs[1].toFixed(1),
      '-5.5',
      'Expected buyer to spend original price plus tax'
    );
    assert.strictEqual(
      accountDkcBalanceDiffs[2].toFixed(1),
      '0.5',
